...
const TestimonialCard: React.FC<TestimonialProps> = ({ quote, author, role }) => {
  return (
    <div className="glass-card rounded-xl p-5 md:p-6 transition-all duration-300 hover:border-dizai-neon-green hover:scale-105 h-full">
      <div className="flex mb-4">
        {[...Array(5)].map((_, i) => (
          <svg key={i} className="w-4 h-4 md:w-5 md:h-5 text-dizai-neon-green" fill="currentColor" viewBox="0 0 20 20">
          ...
          {testimonials.map((_, index) => (
            <button 
              ...
              className={`w-3 h-3 rounded-full transition-colors ${
                index === activeIndex ? 'bg-dizai-neon-green' : 'bg-dizai-light-purple/30'
              }`}
    ...
    <section id="testimonials" className="py-12 md:py-24 px-4 md:px-8">
      ...
      <div className="mt-10 md:mt-16 text-center">
        <div className="inline-flex items-center justify-center space-x-2 bg-dizai-deep-purple/30 rounded-full py-2 px-4 border border-dizai-light-purple/20 animate-pulse-subtle">
          ...
          <div className="flex">
            {[...Array(5)].map((_, i) => (
              <svg key={i} className="w-3 h-3 md:w-4 md:h-4 text-dizai-neon-green" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
            ))}
          </div>
